<script setup lang="ts">
    ...
    import useChads from '../../hooks/useChads';
    import Chads, { alias, fields } from '../../models/Chads';
    import { useActiveDoctorStore } from '../../stores/activeDoctorStore';
import EntityForm from '../../components/EntityForm.vue';

    const { selectedPatient } = useActiveDoctorStore();
    const { create, update, get, remove, calculate, generateDummy, nonExistentMessage } = useChads();

    const error = ref<string>();
    const isErrorCritical = computed<boolean>(
        () => error.value !== nonExistentMessage
    );
    const scale = ref<Chads>();
    ...
    const onUpdate = async (newVal: Chads) => {
        scale.value = newVal;
        const result = await update(selectedPatient.value?.id || -1, scale.value as Chads);
...
<template>
    <v-dialog v-model="isFormDisplayed" max-width="50%">
        <v-sheet class="rounded pa-5 mt-10">
            <entity-form
                @submit="onUpdate($event as unknown as Chads)"
                :fields="fields"
                :alias="alias"
                :defaults="scale"
            >
